...
class BigVGAN(
    ...
    def __init__(self, h: AttrDict, use_cuda_kernel: bool = False):
        ...
        self.degraded_sampling_rate = h.get("degraded_sampling_rate")

        self.sampling_rate_types = h.get("sampling_rate_types")

        self.use_reference_encoder = h.get("use_reference_encoder", False)
        ...
        if self.degraded_sampling_rate is not None:
            self.preencoder = resblock_class(
                h, h.num_mels, 7, [1, 3, 5], activation=h.activation
            )
        else:
            self.preencoder = None

        if self.sampling_rate_types is not None:
            self.sampling_rate_embed = nn.Embedding(
                len(self.sampling_rate_types), h.num_mels
            )
        else:
            self.sampling_rate_embed = None

        if self.use_reference_encoder:
            self.reference_encoder = resblock_class(
                h, h.num_mels, 7, [1, 3, 5], activation=h.activation
            )
        else:
    ...
    def forward(
        ...
        sampling_rate_types: Optional[torch.Tensor] = None,
        ...
        if self.preencoder is not None:
            ...
            if target_size is None:
                ...
                target_sample_length = duration * self.h.sampling_rate
                target_size = int(target_sample_length / self.h.hop_size / speed)
            x = torch.nn.functional.interpolate(x, size=target_size, mode="linear")

        if self.sampling_rate_embed is not None:
            if isinstance(sampling_rate_types, int):
                sampling_rate_types = torch.LongTensor(
                    [self.h.sampling_rate_types[str(sampling_rate_types)]]
                ).to(x.device)

            sampling_rate_types = self.sampling_rate_embed(sampling_rate_types)
            sampling_rate_types = sampling_rate_types.unsqueeze(2)
            x = x + sampling_rate_types

        if self.reference_encoder is not None:
            reference_x = self.reference_encoder(reference_x)
            reference_x = torch.mean(reference_x, axis=-1, keepdim=True)
            x = x + reference_x
        ...
        **model_kwargs,
        ...
        if os.path.isdir(pretrained_model_name_or_path):
            ...
            config_file = hf_hub_download(
                repo_id=pretrained_model_name_or_path,  # 여기도 변경
        ...
        # 여기도 model_id를 pretrained_model_name_or_path로 변경
        if os.path.isdir(pretrained_model_name_or_path):
            print("Loading weights from local directory")
            model_file = os.path.join(
                pretrained_model_name_or_path, "bigvgan_generator.pt"
            )
        else:
            print(f"Loading weights from {pretrained_model_name_or_path}")
